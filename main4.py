...
NOTE: THIS CODE IS IDENTICAL TO LR3/main.py
"""
...
def train_on_device(device: str, train_data, test_data, log_dir: str) -> None:
    """Train the model on the specified device (CPU or GPU).
    ...
    with tf.device(f"/{device}:0"):
        ...
        tf.saved_model.save(model, f"model_{device}/")
...
def main() -> None:
    ...
    # Train on CPU
    cpu_log_dir = os.path.join("tensorboard", "fit_cpu") + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    train_on_device("CPU", train_data, test_data, cpu_log_dir)

    # Train on GPU
    gpu_log_dir = os.path.join("tensorboard", "fit_gpu") + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    train_on_device("GPU", train_data, test_data, gpu_log_dir)
